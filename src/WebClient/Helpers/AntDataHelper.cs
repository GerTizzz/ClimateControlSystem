...
namespace WebClient.Helpers
{
    public static class AntDataHelper
    {
        ...
        public static IEnumerable<GraphicData> GetTemperature(List<ForecastDto> monitorings, ConfigsDto config)
        {
            var temperatureData = new List<GraphicData>();

            for (var i = 0; i < monitorings.Count; i++)
            {
                var graphicsData = GetTemperatureGraphicData(monitorings[i], config);
        ...
        public static IEnumerable<GraphicData> GetHumidity(List<ForecastDto> monitorings, ConfigsDto config)
        {
            try
            {
                ...
                for (var i = 0; i < monitorings.Count; i++)
                {
                    var graphicsData = GetHumidityGraphicData(monitorings[i], config);
