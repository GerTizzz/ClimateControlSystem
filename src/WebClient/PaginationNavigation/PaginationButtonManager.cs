...
public sealed class PaginationButtonManager
{
    ...
    private readonly int _lastPageNumber;
    ...
    {
        if (recordsCount < 0) throw new ArgumentException($"{nameof(PaginationButtonManager)} -> ctor: {nameof(recordsCount)}");
        if (recordsPerPage < 1) throw new ArgumentException($"{nameof(PaginationButtonManager)} -> ctor: {nameof(recordsPerPage)}");
    ...
    private int GetNextPageNumber(BasePaginationButton nextPage)
    {
        ...
        return nextPage.PageNumber < StartPageNumber ? StartPageNumber : nextPage.PageNumber;
    }
    ...
    private VisiblePagesRange DefineStartAndEndIndices()
    {
        ...
        if (endPageRangeNumber <= _lastPageNumber)
        {
