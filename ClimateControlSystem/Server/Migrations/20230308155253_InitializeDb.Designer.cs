// <auto-generated />
using System;
using ClimateControl.Server.Persistence.Context;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace ClimateControl.Server.Migrations
{
    [DbContext(typeof(PredictionsDbContext))]
    [Migration("20230308155253_InitializeDb")]
    partial class InitializeDb
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.AccuracysEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<float>("Humidity")
                        .HasColumnType("real");

                    b.Property<float>("Temperature")
                        .HasColumnType("real");

                    b.HasKey("Id");

                    b.ToTable("Accuracies");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.ActualDataEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<float>("Humidity")
                        .HasColumnType("real");

                    b.Property<float>("Temperature")
                        .HasColumnType("real");

                    b.HasKey("Id");

                    b.ToTable("ActualData");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.ConfigsEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<float>("LowerHumidityWarningLimit")
                        .HasColumnType("real");

                    b.Property<float>("LowerTemperatureWarningLimit")
                        .HasColumnType("real");

                    b.Property<int>("PredictionTimeIntervalSeconds")
                        .HasColumnType("int");

                    b.Property<float>("UpperHumidityWarningLimit")
                        .HasColumnType("real");

                    b.Property<float>("UpperTemperatureWarningLimit")
                        .HasColumnType("real");

                    b.HasKey("Id");

                    b.ToTable("Configs");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            LowerHumidityWarningLimit = 14f,
                            LowerTemperatureWarningLimit = 16f,
                            PredictionTimeIntervalSeconds = 5,
                            UpperHumidityWarningLimit = 22f,
                            UpperTemperatureWarningLimit = 24f
                        });
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.FeaturesDataEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<float>("AirDryTemperatureOutside")
                        .HasColumnType("real");

                    b.Property<float>("AirHumidityOutside")
                        .HasColumnType("real");

                    b.Property<float>("AirWetTemperatureOutside")
                        .HasColumnType("real");

                    b.Property<float>("ClusterLoad")
                        .HasColumnType("real");

                    b.Property<float>("ClusterTemperature")
                        .HasColumnType("real");

                    b.Property<float>("CoolingValue")
                        .HasColumnType("real");

                    b.Property<float>("CpuUsage")
                        .HasColumnType("real");

                    b.Property<float>("Humidity")
                        .HasColumnType("real");

                    b.Property<float>("Temperature")
                        .HasColumnType("real");

                    b.Property<float>("WindDirection")
                        .HasColumnType("real");

                    b.Property<float>("WindEnthalpy")
                        .HasColumnType("real");

                    b.Property<float>("WindSpeed")
                        .HasColumnType("real");

                    b.HasKey("Id");

                    b.ToTable("FeaturesData");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.MicroclimatesEventsEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<float?>("Humidity")
                        .HasColumnType("real");

                    b.Property<float?>("Temperature")
                        .HasColumnType("real");

                    b.HasKey("Id");

                    b.ToTable("MicroclimatesEvents");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.MonitoringsEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("AccuracyId")
                        .HasColumnType("int");

                    b.Property<int?>("ActualDataId")
                        .HasColumnType("int");

                    b.Property<int?>("MicroclimatesEventId")
                        .HasColumnType("int");

                    b.Property<int?>("PredictionId")
                        .HasColumnType("int");

                    b.Property<DateTimeOffset?>("TracedTime")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("AccuracyId");

                    b.HasIndex("ActualDataId");

                    b.HasIndex("MicroclimatesEventId");

                    b.HasIndex("PredictionId");

                    b.ToTable("Monitorings");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.PredictionsEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("FeaturesId")
                        .HasColumnType("int");

                    b.Property<float>("Humidity")
                        .HasColumnType("real");

                    b.Property<float>("Temperature")
                        .HasColumnType("real");

                    b.HasKey("Id");

                    b.HasIndex("FeaturesId");

                    b.ToTable("Predictions");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.UserEntity", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<byte[]>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("varbinary(max)");

                    b.Property<byte[]>("PasswordSalt")
                        .IsRequired()
                        .HasColumnType("varbinary(max)");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "admin",
                            PasswordHash = new byte[] { 234, 67, 146, 201, 134, 164, 86, 202, 125, 217, 174, 99, 230, 69, 196, 32, 223, 130, 86, 2, 110, 245, 35, 7, 159, 20, 84, 62, 49, 84, 81, 28, 175, 203, 198, 202, 128, 63, 128, 15, 96, 135, 210, 4, 252, 15, 252, 17, 150, 160, 104, 243, 99, 40, 181, 210, 193, 226, 14, 26, 229, 165, 150, 197 },
                            PasswordSalt = new byte[] { 29, 90, 245, 35, 83, 27, 162, 74, 226, 234, 171, 134, 93, 187, 246, 80, 193, 193, 90, 50, 37, 118, 116, 254, 107, 30, 200, 72, 10, 31, 43, 139, 58, 135, 118, 189, 5, 99, 211, 203, 0, 84, 81, 146, 28, 164, 132, 63, 61, 143, 124, 25, 66, 231, 99, 189, 203, 55, 91, 105, 23, 169, 254, 10, 20, 179, 147, 58, 198, 70, 204, 60, 221, 77, 160, 128, 50, 190, 189, 205, 83, 48, 107, 183, 51, 48, 173, 248, 28, 230, 153, 194, 13, 108, 51, 123, 87, 228, 62, 31, 167, 11, 30, 180, 130, 172, 254, 241, 22, 7, 150, 212, 195, 48, 144, 92, 52, 199, 221, 202, 91, 200, 83, 109, 66, 70, 223, 200 },
                            Role = "Admin"
                        });
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.MonitoringsEntity", b =>
                {
                    b.HasOne("ClimateControl.Server.Resources.Repository.TablesEntities.AccuracysEntity", "Accuracy")
                        .WithMany()
                        .HasForeignKey("AccuracyId");

                    b.HasOne("ClimateControl.Server.Resources.Repository.TablesEntities.ActualDataEntity", "ActualData")
                        .WithMany()
                        .HasForeignKey("ActualDataId");

                    b.HasOne("ClimateControl.Server.Resources.Repository.TablesEntities.MicroclimatesEventsEntity", "MicroclimatesEvent")
                        .WithMany()
                        .HasForeignKey("MicroclimatesEventId");

                    b.HasOne("ClimateControl.Server.Resources.Repository.TablesEntities.PredictionsEntity", "Prediction")
                        .WithMany()
                        .HasForeignKey("PredictionId");

                    b.Navigation("Accuracy");

                    b.Navigation("ActualData");

                    b.Navigation("MicroclimatesEvent");

                    b.Navigation("Prediction");
                });

            modelBuilder.Entity("ClimateControl.Server.Resources.Repository.TablesEntities.PredictionsEntity", b =>
                {
                    b.HasOne("ClimateControl.Server.Resources.Repository.TablesEntities.FeaturesDataEntity", "Features")
                        .WithMany()
                        .HasForeignKey("FeaturesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Features");
                });
#pragma warning restore 612, 618
        }
    }
}
